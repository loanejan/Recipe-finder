...
export default function RecipePage() {
  ...
  if (loading) {
    return <main className="p-6 max-w-3xl mx-auto text-brand-muted text-sm">Loading recipe…</main>;
  }
  ...
  if (error || !recipe) {
    return (
      <main className="p-6 max-w-3xl mx-auto text-red-400 text-sm space-y-4">
        ...
        <Link to="/" className="inline-block text-brand-primary text-xs hover:underline">
          ← Back to recipes
        </Link>
      </main>
    );
            ...
            <div className="space-y-2">
              ...
              <ul className="text-sm text-brand-text/80 space-y-1">
                {recipe.ingredients.map((ing) => (
                  <li key={ing.id} className="flex items-start gap-2 leading-snug">
                    <span className="text-brand-primary text-xs mt-[3px]">•</span>
